...
public class BarberianBattleState : BattleState
{
    private SkillAgent agent;
    private Barberian barberian;
    public BarberianBattleState(Barberian character,
...
/*                if (TargetIsTooClose())
    ...
    {
        character.animationController.SetTrigger(CharacterAnimParameters.Attack);
        await Task.Delay(700);
        SkillParameters parameters = new SkillParameters(character.target, barberian.shootPoint);
        await agent.UseSkill(SkillName.AxeSlashBarberian, parameters);
        Debug.Log("Barberian attack");
